...
// Source File Name:   ItemToolMeter.java
			...
			float hitX, float hitY, float hitZ)
	{
		...
		if (((tileEntity instanceof IEnergySource) || (tileEntity instanceof IEnergyConductor) || (tileEntity instanceof IEnergySink)) && IC2.platform.isSimulating())
		{
			...
			} else
			{
				...
				IC2.platform.messagePlayer(entityplayer, (new StringBuilder()).append("Measured power [EU/t]: ").append(powerFormat.format(deltaSunken)).append(" in ").append(powerFormat.format(deltaEmitted)).append(" out ").append(powerFormat.format(deltaSunken - deltaEmitted)).append(" gain").append(" (avg. over ").append(measurePeriod).append(" ticks)").toString(), new Object[0]);
			}
