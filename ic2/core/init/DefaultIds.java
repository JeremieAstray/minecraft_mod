...
// Source File Name:   DefaultIds.java

package ic2.core.init;

import java.util.HashMap;

// Referenced classes of package ic2.core.init:
//			InternalName
...
public final class DefaultIds
{
	...
	public static int get(InternalName name)
	{
		if (!idMap.containsKey(name))
			throw new IllegalArgumentException((new StringBuilder()).append("default id for ").append(name).append(" not registered").toString());
		else
	...
	private static void add(InternalName name, int id)
	{
		...
		{
			throw new RuntimeException((new StringBuilder()).append("duplicate default id: ").append(id).toString());
		} else
		{
