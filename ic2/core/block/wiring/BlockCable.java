...
// Source File Name:   BlockCable.java
...
//			TileEntityCable, TileEntityCableDetector, TileEntityCableSplitter

public class BlockCable extends BlockMultiID
{

	private static final Direction directions[] = Direction.values();
	private static final int coloredMetas[] = {
		0, 3, 4, 6, 7, 8, 9
	};
	public boolean enableBreakBlock;
	...
	public void registerIcons(IconRegister iconRegister)
	{
		...
		coloredTextures = new Icon[coloredMetas.length][90];
		for (int index = 0; index < coloredMetas.length; index++)
		{
			int meta = coloredMetas[index];
			for (int color = 1; color < 16; color++)
			{
				String name = (new StringBuilder()).append("ic2:").append(getTextureFolder()).append("/").append(getTextureName(meta)).append(".").append(Util.getColorName(color).name()).toString();
				for (int side = 0; side < 6; side++)
				{
					String subName = (new StringBuilder()).append(name).append(":").append(side).toString();
					net.minecraft.client.renderer.texture.TextureAtlasSprite texture = new BlockTextureStitched(subName, side);
					coloredTextures[index][(color - 1) * 6 + side] = texture;
	...
	public Icon getBlockTexture(IBlockAccess iBlockAccess, int x, int y, int z, int side)
	{
		...
		if (cable.foamed == 0)
			...
			} else
			{
				...
				return coloredTextures[index][(cable.color - 1) * 6 + side];
			}
		if (cable.foamed == 1)
		...
		referencedBlock = Block.blocksList[cable.retextureRefId[side]];
	...
	public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 origin, Vec3 absDirection)
	{
		...
		double halfThickness = tileEntityCable.foamed <= 0 ? (double)tileEntityCable.getCableThickness() / 2D : 0.5D;
		boolean hit = false;
		...
		if (halfThickness < 0.5D)
		{
			...
			for (int i$ = 0; i$ < len$; i$++)
			{
				...
				{
				case 1: // '\001'
					bbox = AxisAlignedBB.getAABBPool().getAABB(x, ((double)y + 0.5D) - halfThickness, ((double)z + 0.5D) - halfThickness, (double)x + 0.5D, (double)y + 0.5D + halfThickness, (double)z + 0.5D + halfThickness);
					break;
				...
				case 3: // '\003'
					bbox = AxisAlignedBB.getAABBPool().getAABB(((double)x + 0.5D) - halfThickness, y, ((double)z + 0.5D) - halfThickness, (double)x + 0.5D + halfThickness, (double)y + 0.5D, (double)z + 0.5D + halfThickness);
					break;
				...
				case 5: // '\005'
					bbox = AxisAlignedBB.getAABBPool().getAABB(((double)x + 0.5D) - halfThickness, ((double)y + 0.5D) - halfThickness, z, (double)x + 0.5D + halfThickness, (double)y + 0.5D, (double)z + 0.5D);
					break;
	...
	public AxisAlignedBB getCommonBoundingBoxFromPool(World world, int x, int y, int z, boolean selectionBoundingBox)
	{
		...
		double halfThickness = cable.foamed != 1 || !selectionBoundingBox ? (double)cable.getCableThickness() / 2D : 0.5D;
	...
	public float getExplosionResistance(Entity exploder, World world, int x, int y, int z, double src_x, 
			double src_y, double src_z)
	{
		TileEntity te = world.getBlockTileEntity(x, y, z);
		if (te instanceof TileEntityCable)
		{
	...
	{
		List ret = getBlockDropped(world, x, y, z, world.getBlockMetadata(x, y, z), 0);
		if (ret.isEmpty())
			return null;
		else
	...
	public int getComparatorInputOverride(World par1World, int par2, int par3, int par4, int par5)
	{
		...
		if (te != null && te.getClass() == ic2/core/block/wiring/TileEntityCableDetector)
		{
			TileEntityCableDetector tec = (TileEntityCableDetector)te;
			return (int)Math.floor(((float)EnergyNet.getForWorld(par1World).getTotalEnergyEmitted(te) / ((float)tec.getConductorBreakdownEnergy() - 1.0F)) * 15F);
		} else
		{
			return 0;
		}
